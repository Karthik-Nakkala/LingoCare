...
import { analyzeCurriculumQuality, validateCurriculumOutput } from './curriculumValidation.js';
...
const retryableNetworkCodes = new Set(['ECONNRESET', 'ETIMEDOUT', 'ECONNREFUSED', 'EAI_AGAIN', 'ENOTFOUND']);
...
async function generateWithRetry(ai, request) {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    try {
      ...
      const code = error?.cause?.code || error?.code || (error?.status?.toString && error.status.toString());
...
function mapGeminiError(error) {
  ...
  if (retryableNetworkCodes.has(code)) {
    return {
      ...
      userMessage: 'AI generation is temporarily unavailable. Please check your connection and try again.',
    };
  }
...
app.post('/api/generate-curriculum', async (req, res) => {
  ...
  if (!document.fullText.trim()) {
    return clientError(res, 400, 'NO_EXTRACTABLE_TEXT', "We couldn't find readable text in this PDF.");
  }
  ...
  if (!process.env.GEMINI_API_KEY) {
    ...
    return clientError(res, 503, 'AI_NOT_CONFIGURED', 'AI generation is currently unavailable.');
  }
  ...
  try {
    ...
    try {
      ...
      return clientError(res, 422, 'MALFORMED_AI_OUTPUT', "AI returned an invalid response format. Please try again.");
    }
    ...
    return clientError(res, mapped.status, mapped.errorCode, mapped.userMessage);
  } finally {
    clearTimeout(timeoutHandle);
    isGenerating = false;
  }
...
app.use((err, req, res, next) => {
  ...
  return clientError(res, 500, 'INTERNAL_SERVER_ERROR', 'An unexpected server error occurred. Please try again.');
});

const port = Number(process.env.PORT || process.env.API_PORT || 8788);
app.listen(port, () => console.log(`Lingocare engine running on port ${port} (Model: ${GEMINI_MODEL})`));
